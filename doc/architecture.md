...
Why does this matter for Big Bunny? Because each customer gets their own encryption key derived from the master key using HKDF (a key derivation function). The derivation formula incorporates both the site identifier and the customer ID, so customer A and customer B get completely different keys even though they share the same master key. Additionally, the same customer on different sites (like "nyc01" vs "sfo01") gets different keys, binding store IDs to their originating site. This provides cryptographic isolation—customer A literally cannot decrypt customer B's store IDs, and store IDs cannot be used across sites, even if both sites share the same master key.

The plaintext inside the encrypted store ID contains two pieces of information: a shard ID (random bytes used for routing) and a unique identifier (more random bytes to prevent collisions). The site is not stored in the plaintext because it's already cryptographically bound through the HKDF key derivation. When someone presents a store ID, Big Bunny decrypts it using a key derived from the site and customer ID in the request. If decryption fails, it's either an invalid store ID, a store ID for a different customer, or a store ID from a different site. Either way, the request is rejected.
