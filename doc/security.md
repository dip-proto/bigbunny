...
The algorithm is AES-256-SIV, which stands for Synthetic Initialization Vector. This variant of AES is designed to be misuse-resistant. With standard AES-GCM, accidentally reusing a nonce catastrophically leaks your encryption key. With AES-SIV, reusing a nonce only reveals whether two plaintexts are identical, which is much less severe. This property matters for Big Bunny because each customer gets their own encryption key derived from the master key, and the derivation needs to be deterministic and repeatable.
