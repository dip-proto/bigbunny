...
When encrypting a store ID, Big Bunny takes the plaintext (which contains a shard ID for routing and a unique identifier), derives the customer-specific key, and encrypts using AES-SIV. The result is a ciphertext that only that customer can decrypt. If customer A tries to decrypt customer B's store ID, the decryption fails because they're using different derived keys.
...
Inside the plaintext, before encryption, there are two pieces of information. The shard ID is random bytes used for deterministic replica placement. The unique identifier is more random bytes to prevent collisions. When combined and encrypted, these create an opaque token that clients can't tamper with or forge. The site is not stored in the plaintext because it's already cryptographically bound through the key derivation (HKDF includes the site), making any redundant plaintext storage unnecessary.
...
**Cross-site access**: Someone tries to use a store ID from site "nyc01" on site "sfo01". This fails because the site is incorporated into the HKDF key derivation. The key derived for the same customer on different sites will be different, causing decryption to fail. This prevents accidental cross-site usage and ensures store IDs are bound to their originating site.
...
**Information leakage**: An attacker tries to infer information from store IDs or error messages. Store ID length is now fixed (site is not stored in the plaintext), so it reveals nothing about the deployment. Deterministic encryption means identical plaintexts produce identical ciphertexts, but the unique identifier in each store ID prevents this from being useful. Error messages are generic—both invalid store IDs and wrong-customer IDs return "400 Bad Request" without details. Timing attacks are theoretically possible, but AES-SIV is implemented in constant time in the crypto libraries Big Bunny uses.
